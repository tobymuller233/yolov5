...
original_image = cv2.imread("datasets/VOC/val/images/2007_002142.jpg")
original_image = cv2.resize(original_image, (640, 480))
# 生成模拟特征图数据 [1, 6, 128, 80, 60]
stage = "23"
feature_tensor = np.load(f"runs/detect/exp90/2007_002142/stage{stage}_C3_features.npy")
...
merged_features = feature_tensor.mean(axis=0)
...
feature_map = upsampled.squeeze().numpy()  # [480, 640]
# 设置阈值
threshold = 0 * feature_map.max()
# 将小于阈值的值设置为最小值
feature_map[feature_map < threshold] = feature_map.min()
# 归一化到[0,1]范围
normalized = (feature_map - feature_map.min()) / (feature_map.max() - feature_map.min())
# 叠加特征图热力图
heatmap = axes.imshow(original_image, alpha=1)
# 叠加特征图热力图
heatmap = axes.imshow(normalized, cmap='jet', alpha=0.55, interpolation='antialiased')
# 添加颜色条
...
plt.subplots_adjust(wspace=0.05, hspace=0.2)
# plt.show()
plt.savefig(f"thesis_results/2142_det_yolodist_n/feature_maps_sum_{stage}.png", dpi=300, bbox_inches='tight')
