...
def add_quant_op(module, layer_counter, a_bits=8, w_bits=8,
                 quant_inference=False, all_positive=False, per_channel=False, batch_init = 20, device="cpu"):
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
            ...
            if layer_counter[0] >= 1: #第一层也量化
                if child.bias is not None:
                    quant_conv = QuantConv2d(child.in_channels, child.out_channels,
                                             ...
                                             all_positive=all_positive, per_channel=per_channel, batch_init = batch_init).to(device)
            ...
            add_quant_op(child, layer_counter, a_bits=a_bits, w_bits=w_bits,
                         ...
                         per_channel=per_channel, batch_init = batch_init, device=device)


def prepare(model, inplace=False, a_bits=8, w_bits=8, quant_inference=False,
            all_positive=False, per_channel=False, batch_init = 20, device="cpu"):
    if not inplace:
        model = copy.deepcopy(model)
    layer_counter = [0]
    add_quant_op(model, layer_counter, a_bits=a_bits, w_bits=w_bits,
                 quant_inference=quant_inference, 
                 all_positive=all_positive, per_channel=per_channel, batch_init = batch_init, device=device)
    return model